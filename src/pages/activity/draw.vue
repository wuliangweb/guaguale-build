<template>
    <view class="conbox">
        <view class="container">
            <!-- 背景 -->
            <image src="../../static/bg.png" class="cont" mode=""></image>
            <image src="../../static/caidai.png" class="caidai" mode=""></image>
            <view class="main" style="padding-top: 50upx;">
                <view class="canvas-container" style="position: absolute; left: 50%; top: 50%; transform: translate(-50%, -50%);">
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      playReward()
      {
        let index =  Math.floor(Math.random() * (this.list.length - 1)), duration = 4000
        this.animation(index, duration)

        setTimeout(() => {
          uni.showModal({content: this.list[index].isNoPrize ? '抱歉，您未中奖' : `恭喜，抽中${this.list[index].name}`})
          this.btnDisabled = '';
